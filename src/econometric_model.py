...
class AutoRegModelsDf:
    ...
    def fit_models(self):
        ...
        for title in self.df.columns:
            data = self.df[title]
            model = sm.tsa.AutoReg(endog=data, lags=self.max_lag, exog=self.exog_data , old_names=False)
            results = model.fit()
...
class GJRGARCH:
    ...
    def fit(self):
        ...
        returns = self.df_returns['CAIG1U5 INDEX']
        market_residuals = self.df_market_residuals.values
